...
export default function ModuleSession() {
  ...
  useEffect(() => {
    const sectionsNeedingUpdate = calculateProgress.sections.details.filter(
      (section: any) => section.needsProgressUpdate
    );

    if (sectionsNeedingUpdate.length === 0) return;

    setModuleProgress((prev) => {
      const newSections = new Map(prev.sections);

      sectionsNeedingUpdate.forEach((section: any) => {
        const existingProgress = newSections.get(section.sectionId);
