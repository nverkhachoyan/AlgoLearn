  ...
  if (!isAuthenticated || !token) {
    ...
    return (
      <View style={[styles.container, { backgroundColor: '#00FF00' }]}>
        <Text style={{ color: '#000000', fontSize: 20 }}>Not authenticated. Please sign in.</Text>
        <Text style={{ color: '#000000', fontSize: 16 }}>
          Debug: {JSON.stringify({ isAuthenticated, hasToken: !!token }, null, 2)}
        </Text>
      </View>
    );
  }
  ...
  if (!user) {
    ...
    return (
      <View style={[styles.container, { backgroundColor: '#0000FF' }]}>
        ...
        <Text style={{ color: '#FFFFFF', marginTop: 16, fontSize: 20 }}>Loading user data...</Text>
      </View>
    );
  }
