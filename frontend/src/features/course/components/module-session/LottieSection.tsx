...
// Import our utility functions and hooks
...
import { AnimationHeader } from "@/src/components/common/AnimationHeader";
...
export const LottieSection = memo(({ content, colors }: LottieSectionProps) => {
  ...
  // State for animation source and loading
  ...
  // Text color filters for the animation
  const textColorFilters = [
    { keypath: "**.Arrow 1", color: dark ? "white" : "black" },
  ];
  ...
  const {
    ...
    onSpeedChange: (newSpeed) => {
      // Apply speed change to animation
  ...
  // Load the animation on mount or when source changes
  useEffect(() => {
    async function initAnimation() {
      setIsLoading(true);

      const result = await loadLottieAnimation(
        content.source,
        content.fallbackUrl
      );
  ...
  useEffect(() => {
    if (animationRef.current && !isLoading && !usingFallback) {
      // If autoplay should be false, force pause after initialization
